...
import warnings
import os
import csv  # Keep csv for potential error logging or fallback
import psycopg2
import datetime
from bs4 import BeautifulSoup
from dotenv import load_dotenv # To load environment variables from a .env file
import time
import pandas as pd # Keep pandas for date conversion utility
import argparse # Added for command-line arguments
import re # Added for parsing season/year from title
from urllib.parse import urljoin # Added for constructing full URLs

warnings.filterwarnings('ignore')

# --- Load Environment Variables (Optional: from a .env file) ---
# Create a .env file in the same directory with your DB credentials:
# DB_HOST=your_host
...
# --- DEBUG PRINTS --- (Removing these now)
# print(f"DEBUG: DB_HOST = {DB_HOST}")
# print(f"DEBUG: DB_NAME = {DB_NAME}")
# print(f"DEBUG: DB_USER = {DB_USER}")
# print(f"DEBUG: DB_PASSWORD is {'set' if DB_PASSWORD else 'NOT set'}") # Avoid printing password
...
def convert_start_date(date_str):
    ...
    try:
        ...
        # Try YYYYMMDD format (e.g., 20240108)
        try:
...
def parse_int_field(value_str):
    ...
    try:
        ...
        # print(f"Warning: Could not convert to int: {value_str}") # Optional: Can be verbose
        return None
    ...
    try:
        ...
        def get_info(label):
            ...
            if element and element.parent:
                 # The value is usually the next sibling text node, stripped
                 value = element.next_sibling
                 if value and isinstance(value, str):
                     cleaned_value = value.strip()
                     if cleaned_value:
                        return cleaned_value
                 # Sometimes it might be inside an <a> tag sibling
                 if element.find_next_sibling('a'):
                     return element.find_next_sibling('a').get_text(strip=True)
            return 'N/A' # Return N/A if not found for consistency
...
def scrape_seasonal_anime(target_year=None, target_season=None):
    ...
    # --- Determine Season and Year (if not provided) ---
    current_season = target_season
    current_year = target_year
    if not current_year or not current_season:
        # Try to parse from title or h1 if not provided
        page_title = soup.title.string if soup.title else ''
        h1_text = soup.find('h1').get_text(strip=True) if soup.find('h1') else ''
        # Example Regex: busca "Season: (Winter|Spring|Summer|Fall) (\d{4})" ou "Currently Airing.*(Winter|Spring|Summer|Fall) (\d{4})"
        ...
        if title_match:
            ...
            print(f"Determined current season/year from title: {current_season.capitalize()} {current_year}")
             ...
             # Keep them as None if undetectable
    ...
    for i, anime in enumerate(anime_containers):
        ...
        # --- (End Genre/Theme List Extraction) --- 
        ...
        anime_record = {
            ...
            'year': current_year,
            **individual_details # Merge details from individual page
        }
        # Store the genre/theme names separately for insertion later
    ...
    # --- Database Insertion (Revised Logic) ---
    ...
    inserted_count = 0
    skipped_count = 0
    start_db_time = time.time() # Time DB operations
    ...
    try:
        with conn.cursor() as cur:
                ...
                try:
                    ...
                    if result:
                        ...
                        for theme_name in theme_names:
                            ...
                            # Link anime history to theme
                            cur.execute(link_theme_sql, (history_scrape_id, theme_id))

                    else:
                        # ON CONFLICT happened, row was not inserted
                        skipped_count += 1
                    ...
                    # Decide if you want to continue with the next anime or stop
            ...
            # Commit the entire batch transaction if no fatal errors occurred (and break wasn't hit)
            conn.commit()
...
if __name__ == "__main__":
    ...
    if success:
        ...
        exit(1) # Exit with error code if scraping failed 